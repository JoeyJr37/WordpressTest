...
if ( ! function_exists( 'test_project_setup' ) ) :
	/**
	...
	function test_project_setup() {
		 ...
		 * If you're building a theme based on Test Project, use a find and replace
		 * to change 'test-project' to the name of your theme in all the template files.
		 */
		...
		add_theme_support( 'custom-background', apply_filters( 'test_project_custom_background_args', array(
...
function test_project_scripts() {
	wp_enqueue_style( 'test-project-style', get_stylesheet_uri() );

	wp_enqueue_script( 'test-project-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'test-project-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
